...
No new DB columns. Reuse the existing `settings` JSON blob on `wp_ai_forms`:
...
- Registers a query var: `wp_ai_form_preview`.
...
```html
<style id="wpaif-css-{uuid}">
  .wpaif-form[data-form-uuid="{uuid}"] {
...
- Initial mount: `iframe src="/?wp_ai_form_preview={id}"`.
...
- [ ] Anonymous visitors hitting `/?wp_ai_form_preview={id}` get a 403.
